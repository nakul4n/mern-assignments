...
class BookController {
    ...
    async getReview(request,response){
        try{ console.log("req",request.params.id);
            const book=await service.getReview(request.params.id);
            response.json(book)
        }catch(e){
            response.status(e.status).json(e.details);
        }
    ...
    async addReview(request,response){
        try{
            const result=await service.addReview(request.params.id,request.body);
            response.status(201).json(result);
        }catch(e){
            response.status(e.status).json(e.details);
        }
